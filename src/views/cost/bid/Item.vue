<template>
  <page-header-wrapper>
    <a-card :bordered="false">
          ...
          <a-col :md="24" :sm="24">
            <table>
              ...
              <tr>
                ...
                <th style="width: 10%">分期/阶段</th>
                <th style="width: 15%">专业分判包编号</th>
                <th style="width: 15%">描述</th>
                <th style="width: 20%">范围</th>
                <th style="width: 15%">金额</th>
                <th style="width: 20%">合同</th>
              </tr>
              </thead>
              <tbody>
              <tr v-for="(item,index) in form.tenderPackages" :key="index">
                ...
                <td>{{ getIndustryItem(index).projectName }}</td>
                <td>
                  <a-form-model-item
                    class="simple"
                    style="margin-top: 20px"
                    :prop="'tenderPackages.' + index"
                    :rules="[{required: true, message: '请选择专业分判包'}]"
                  >
                    <a-input
                      :hidden="true"
                ...
                <td>{{ getIndustryItem(index).contractNo }}</td>
              </tr>
              </tbody>
            </table>
          </a-col>
      ...
      <a-row>
        <a-col :md="12" :sm="24">
          <a-button type="success" style="margin-right: 20px" @click="startBPM" :loading="loading.startBPM"
                    v-if="type === 'view' && form.auditStatus === '未审核'">启动审批流程
          </a-button>
          <a-button type="success" style="margin-right: 20px" @click="showBPM" :loading="loading.showBPM"
                    v-if="type === 'view' && (form.auditStatus === '审核中' || form.auditStatus === '已审核')">查看审批流程
          </a-button>
          <a-button type="success" style="margin-right: 20px" @click="showApplyUrl" v-if="costSystemApplyUrl">查看采购申请
          </a-button>
          <a-button type="success" style="margin-right: 20px" @click="exportPurchaseFile"
                    v-if="form.auditStatus === '已审核'" :disabled="disabled2">导出采购
          </a-button>

        </a-col>
        <a-col :md="12" :sm="24">
          <a-button-group style="float: right">
            <a-button :disabled="type === 'view' || disabled" :loading="loading.save" type="success"
                      @click="handleToSave">储存
            </a-button>
      ...
      <industry-list-modal
        ref="industryListModal"
        v-if="this.ProjectGUID"
        :ProjectGUID="this.ProjectGUID"
        :refreshParent="selectRefresh"
      ></industry-list-modal>
    </a-card>
  </page-header-wrapper>
</template>
...
<script>
    ...
    import { addItem, removeItem, Base as BaseService, DIALOGCONFIG } from '@/api/base'
    import moment from 'moment'
    import IndustryListModal from '@/views/cost/bid/modal/IndustryListModal'
    ...
    export default {
        ...
        data () {
            return {
                ...
                rules: {
                    ...
                    needBuyInsurance: [{
                        required: true,
                        message: '请选择需发包方购买工程一切险、第三者责任险、建筑工程团体意外伤害险',
                        trigger: 'change'
                    }],
                    tenderPackages: [],
                    plans: []
        ...
        methods: {
                ...
                if (this.type !== 'add') {
                    ...
                    CostService.bidItem({ Id: this.id }).then(res => {
                        ...
                        if (this.form.tenderPackages) {
                            // 获取下拉分判包列表
                            this.industryItems = JSON.parse(JSON.stringify(this.form.tenderPackages))
                                ...
                                this.form.tenderPackages.forEach(item => {
                                    this.industryItems.forEach(industryItem => {
                                        if (item === industryItem.id && !industryItem.isEditItem) {
                                            this.addTenderPackages.push({
                                                projectTenderPackageId: this.id,
                                                tenderPackageId: item
                                            })
                                        }
                                    })
                                })
                ...
                if (this.type === 'edit') {
                    ...
                    this.industryItems.forEach(industryItem => {
                        if (industryItem.id === item && industryItem.isEditItem) {
                            // 移除的计划放到移除对象
                            this.removeTenderPackages.push({
                                projectTenderPackageId: this.id,
                                tenderPackageId: industryItem.id
                            })
                        }
                    })
                }
                items.splice(index, 1)
                this.industryItems.splice(index, 1)
                this.$forceUpdate()
            },
            delPlan (index) {
            ...
            // 弹出分判包选择框
            showSelect (index) {
                this.$refs.industryListModal.showModal(this.ProjectGUID, this.id, index)
            },
            selectRefresh (item) {
                let isHaving = false
                if (this.form.tenderPackages) {
                    this.form.tenderPackages.forEach(packageItem => {
                ...
                if (!isHaving) {
                    ...
                    this.industryItems[index] = item
                    this.form.tenderPackages[index] = item.id
                    this.$set(this.form, 'tenderPackages', JSON.parse(JSON.stringify(this.form.tenderPackages)))
                    this.$forceUpdate()
                } else {
                    this.$message.error('请不要添加重复的专业分判包')
                }
