...
function getBudgetList (elementInfoId, items) {
  let obj = null
  items.forEach(item => {
    if (item.childs && item.childs.length > 0) {
      const temp = getBudgetList(elementInfoId, item.childs)
      if (temp) { obj = temp }
    } else if (item.elementItem && item.elementItem.childs) {
      const temp = getBudgetList(elementInfoId, item.elementItem.childs)
      if (temp) { obj = temp }
    } else if (item.tradeBudgetItems && item.tradeBudgetItems.length > 0) {
      item.tradeBudgetItems.forEach((budget, index) => {
        if (budget.elementInfoId === elementInfoId) {
            obj = {}
  ...
  items.forEach(item => {
    ...
    if (item.tradeBudgetItems && item.tradeBudgetItems.length > 0) {
      item.tradeBudgetItems.forEach((budget, index) => {
        ...
        costCenters.forEach(center => {
          console.log('center'+center.costCenterId,center)
          const result = getBudgetList(budget.elementInfoId, center.elementItem.childs)
          console.log(result)
