...
<script>
  ...
  import { RoleService } from '@/views/role/role.service'
  ...
  export default {
    ...
    created () {
      const roles = []
      RoleService.projects().then(res => {
        res.result.data.forEach(item => {
          roles.push({
            id: item.moduleFunctionId,
            name: item.moduleFunctionName,
            describe: item.moduleFunctionName
          })
        })
        this.roles = roles
      })
